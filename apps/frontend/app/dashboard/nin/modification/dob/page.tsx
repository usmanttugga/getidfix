...
export default function DOBModificationPage() {
  ...
  const handleFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (oversized.length > 0) { setError('Each file must not exceed 5MB.'); return; }
    setDocuments((prev) => {
              ...
              <div className="space-y-2 mb-3">
                {documents.map((file, index) => (
                  <div key={index} className="flex items-center gap-3 p-2.5 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <button type="button" onClick={() => removeFile(index)} className="p-1 hover:bg-green-100 rounded">
