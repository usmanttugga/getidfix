...
import { CheckCircle, User, AlertCircle, Phone } from 'lucide-react';
      ...
      <div className="max-w-lg">
        <div className="bg-white rounded-xl border border-slate-300 p-6">
          ...
          <div className="flex items-center gap-4 mb-5 p-4 bg-slate-50 rounded-xl">
            ...
            <div><p className="font-semibold text-slate-900">{String(r.fullName || '')}</p><p className="text-sm text-slate-500">Phone: {phone}</p></div>
          </div>
          ...
          <button onClick={() => { setResult(null); setPhone(''); setSlipType(null); }} className="mt-4 w-full py-2 border border-slate-300 text-slate-700 rounded-lg text-sm hover:bg-slate-50">Verify Another</button>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-lg">
      <div className="bg-white rounded-xl border border-slate-300 p-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-xl font-semibold text-slate-900">Verify by Phone Number</h1>
        </div>
        <p className="text-sm text-slate-500 mb-1 ml-10">Look up identity using a registered Nigerian phone number.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">11-digit Nigerian mobile number (070, 080, 081, 090, 091)</p>
            </div>
          </div>
          <button type="submit" disabled={loading || balance < price || phone.length !== 11}
